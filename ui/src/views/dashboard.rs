...
use eframe::egui;
use proxy_vm_core::{VmState, VmKind, GatewayMode, ProxyType, WireGuardParsedConfig, OpenVpnParsedConfig};
use crate::app::{ProxyVmWizardApp, ProxyHopEntry};
...
impl DashboardView {
        ...
        if let Some(role) = app.pending_role_delete.clone() {
            egui::Window::new("⚠ Confirm Delete")
                ...
                .show(ui.ctx(), |ui| {
                    ...
                    ui.add_space(10.0);
                    ui.colored_label(egui::Color32::from_rgb(220, 20, 60), "This action cannot be undone!");
                    ui.add_space(10.0);
                    ui.horizontal(|ui| {
    ...
    fn show_role_card(app: &mut ProxyVmWizardApp, ui: &mut egui::Ui, role: &str) {
        ...
        let disp_vms: Vec<_> = vms.iter().filter(|v| v.kind == VmKind::DisposableApp).collect();
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui.button("🗑").on_hover_text("Delete role and all VMs").clicked() {
                            app.pending_role_delete = Some(role.to_string());
                        }
                        if ui.button("🔧").on_hover_text("Edit gateway configuration").clicked() {
                    ...
                    ui.indent("app_vms", |ui| {
                        for vm in &app_vms {
                            ui.horizontal(|ui| {
                                ...
                                ui.colored_label(status_color, format!("{} {}", status_icon, vm.name));
                    ...
                    ui.indent("disp_vms", |ui| {
                        for vm in &disp_vms {
                            ui.horizontal(|ui| {
                                ...
                                if ui.small_button("⏹").on_hover_text("Stop (will delete)").clicked() {
                                    app.stop_vm(&vm.name);
                                }
                            });
                        }
    ...
    fn show_config_editor(app: &mut ProxyVmWizardApp, ui: &mut egui::Ui, role: &str) {
        egui::Window::new(format!("🔧 Edit Gateway Config: {}", role))
            ...
            .show(ui.ctx(), |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.radio_value(&mut app.config_editor.gateway_mode, GatewayMode::ProxyChain, "Proxy Chain");
                    ui.radio_value(&mut app.config_editor.gateway_mode, GatewayMode::WireGuard, "WireGuard");
                    ui.radio_value(&mut app.config_editor.gateway_mode, GatewayMode::OpenVpn, "OpenVPN");
                });
                ...
                egui::ScrollArea::vertical().max_height(400.0).show(ui, |ui| {
                    match app.config_editor.gateway_mode {
                ...
                ui.checkbox(&mut app.config_editor.restart_after_save, "Restart gateway VM after saving");
    ...
    fn show_proxy_chain_editor(app: &mut ProxyVmWizardApp, ui: &mut egui::Ui) {
            ...
            egui::Frame::group(ui.style())
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.label("Pass:");
                        ui.add(egui::TextEdit::singleline(&mut hop.password).password(true).desired_width(100.0));
                    });
                });
            ui.add_space(3.0);
        }
        ...
        if app.config_editor.proxy_hops.len() < 8
    ...
    fn show_wireguard_editor(app: &mut ProxyVmWizardApp, ui: &mut egui::Ui) {
        ...
        if let Some(role) = &app.editing_role_config.clone() {
            ...
            if !configs.is_empty() {
                ...
                for (filename, parsed) in &configs {
                    ui.horizontal(|ui| {
                        let is_selected = app.config_editor.wireguard_config.config_filename == *filename;
                        ...
                        if let Some(peer) = parsed.peers.first() {
                            if let Some(endpoint) = &peer.endpoint {
                                ui.label(egui::RichText::new(format!("→ {}", endpoint)).small().color(egui::Color32::GRAY));
                            }
                        }
                    });
                }
        ...
        ui.horizontal(|ui| {
            ...
            ui.add(egui::TextEdit::singleline(&mut app.config_editor.wireguard_config.config_filename)
                .desired_width(200.0));
        ...
        if !app.config_editor.wireguard_config.config_filename.is_empty() {
            if let Some(role) = &app.editing_role_config.clone() {
                let role_dir = app.global_config.role_dir(role);
                let config_path = role_dir.join(&app.config_editor.wireguard_config.config_filename);
                    ...
                    egui::Frame::group(ui.style())
                        ...
                        .show(ui, |ui| {
                            ...
                            for (i, peer) in parsed.peers.iter().enumerate() {
                                ui.label(egui::RichText::new(format!("Peer {}: {}", 
                                    i + 1, 
                                    peer.name.as_deref().unwrap_or("Unknown")
                                )).small());
                                if let Some(endpoint) = &peer.endpoint {
                                    ui.label(egui::RichText::new(format!("  Endpoint: {}", endpoint)).small().color(egui::Color32::GRAY));
                                }
                            }
                        });
                }
        ...
        ui.horizontal(|ui| {
            ui.label("Interface:");
            ui.add(egui::TextEdit::singleline(&mut app.config_editor.wireguard_config.interface_name)
                .hint_text("wg0")
                .desired_width(100.0));
        });

        ui.checkbox(&mut app.config_editor.wireguard_config.route_all_traffic, "Route all traffic");
    }
    ...
    fn show_openvpn_editor(app: &mut ProxyVmWizardApp, ui: &mut egui::Ui) {
        ...
        if let Some(role) = &app.editing_role_config.clone() {
            ...
            if !configs.is_empty() {
                ...
                for (filename, parsed) in &configs {
                    ui.horizontal(|ui| {
                        let is_selected = app.config_editor.openvpn_config.config_filename == *filename;
                        if ui.selectable_label(is_selected, filename).clicked() {
        ...
        ui.horizontal(|ui| {
            ui.label("Config file:");
            ui.add(egui::TextEdit::singleline(&mut app.config_editor.openvpn_config.config_filename)
                .desired_width(200.0));
            if ui.button("📂 Import").clicked() {
                if let Some(path) = rfd::FileDialog::new()
                    ...
                    egui::Frame::group(ui.style())
                        ...
                        .show(ui, |ui| {
                            ...
                            if let Some(proto) = &parsed.protocol {
                                ui.label(egui::RichText::new(format!("Protocol: {}", proto)).small());
                            }
                            ui.label(egui::RichText::new(format!("Servers: {}", parsed.remotes.len())).small());
                            ...
                            if parsed.remotes.len() > 5 {
                                ui.label(egui::RichText::new(format!("  ... and {} more", parsed.remotes.len() - 5)).small().color(egui::Color32::GRAY));
                            }
                        });
                }
            }
        ...
        ui.horizontal(|ui| {
            ui.label("Auth file:");
            ui.add(egui::TextEdit::singleline(&mut app.config_editor.openvpn_config.auth_filename)
                .hint_text("(optional)")
                .desired_width(200.0));
        ...
        ui.checkbox(&mut app.config_editor.openvpn_config.route_all_traffic, "Route all traffic");
    }
}
